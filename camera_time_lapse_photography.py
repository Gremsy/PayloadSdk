...
import os
from libs.payload_sdk import PayloadSdkInterface, payload_status_event_t, param_type, time_lapse_capture_sequence_t, camera_mode
...
my_capture = time_lapse_capture_sequence_t.CHECK_STORAGE
...
def onPayloadStatusChanged(event: int, param: list):
    ...
    if payload_status_event_t(event) == payload_status_event_t.PAYLOAD_CAM_CAPTURE_STATUS:
        ...
        if my_capture == time_lapse_capture_sequence_t.CHECK_CAPTURE_STATUS:
            print(f"Got payload capture status: image_status: {param[0]:.2f}, video_status: {param[1]:.2f}")
            
            # If image status is idle, do capture
            if param[0] == 0: 
                my_capture = time_lapse_capture_sequence_t.CHECK_CAMERA_MODE
                print("   ---> Payload is idle, Check camera mode")
            else:
                my_capture = time_lapse_capture_sequence_t.IDLE
                print("   ---> Payload is busy")

        elif my_capture == time_lapse_capture_sequence_t.STOP_CAPTURING_IMAGE:
            if param[0] == 0: 
                my_capture = time_lapse_capture_sequence_t.CHECK_STORAGE
        ...
        if my_capture == time_lapse_capture_sequence_t.CHECK_STORAGE:
            ...
            if param[2] >= 10.0: 
                my_capture = time_lapse_capture_sequence_t.CHECK_CAPTURE_STATUS
        ...
        if my_capture == time_lapse_capture_sequence_t.CHECK_CAMERA_MODE:
            print(f"Got camera mode: {param[0]:.2f}")

            if camera_mode(param[0]) == camera_mode.CAMERA_MODE_IMAGE:
...
def main():
    ...
    while not time_to_exit:
        ...
        if my_capture == time_lapse_capture_sequence_t.IDLE:
            # Wait in idle state
            pass  

        elif my_capture == time_lapse_capture_sequence_t.CHECK_STORAGE:
            my_payload.getPayloadStorage()

        elif my_capture == time_lapse_capture_sequence_t.CHECK_CAPTURE_STATUS:
            my_payload.getPayloadCaptureStatus()

        elif my_capture == time_lapse_capture_sequence_t.CHECK_CAMERA_MODE:
            my_payload.getPayloadCameraMode()

        elif my_capture == time_lapse_capture_sequence_t.CHANGE_CAMERA_MODE:
            my_payload.setPayloadCameraMode(camera_mode.CAMERA_MODE_IMAGE)
            my_capture = time_lapse_capture_sequence_t.CHECK_CAMERA_MODE

        elif my_capture == time_lapse_capture_sequence_t.DO_CAPTURE:
            ...
            my_capture = time_lapse_capture_sequence_t.IMAGE_IN_CAPTURING

        elif my_capture == time_lapse_capture_sequence_t.IMAGE_IN_CAPTURING:
            print("Captured ...")
            time.sleep(interval)  
            time_to_capturing -= interval
            if time_to_capturing <= 0:
                my_capture = time_lapse_capture_sequence_t.STOP_CAPTURING_IMAGE
